...
sub my_app {
    ...
    if( $env->{'psgix.session'}{user_id} ){
        $page .= 'Hi ' . $env->{'psgix.session'}{user_id} . '<br><form id="logout_form" name="logout_form" method="post" action="/logout"><input type="submit" name="submit" id="submit" value="Logout" /></form>';
    }
    else{
